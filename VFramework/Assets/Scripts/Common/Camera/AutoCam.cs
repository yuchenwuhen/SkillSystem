...
namespace VFramework.Common
{
    ...
    {
        [SerializeField] private float m_MoveSpeed = 10; // How fast the rig will move to keep up with target's position
        [SerializeField] private float m_TurnSpeed = 1; // How fast the rig will turn to keep up with target's rotation
        private float m_RollSpeed = 0f;// How fast the rig will roll (around Z axis) to match target's roll.
        ...
        [SerializeField] private Vector2 m_screenXSizeLimit = new Vector2(-float.MaxValue, float.MaxValue);
        public Vector2 ScreenXSizeLimit { get { return m_screenXSizeLimit; } }
        [SerializeField] private Vector2 m_screenYSizeLimit = new Vector2(-float.MaxValue, float.MaxValue);
        public Vector2 ScreenYSizeLimit { get { return m_screenYSizeLimit; } }
        ...
        protected override void FollowTarget(float deltaTime)
        {
            ...
            Vector3 cameraPos = Vector3.Lerp(transform.position, m_Target.position + new Vector3(cameraOffsetX, cameraOffsetY, transform.position.z), deltaTime * m_MoveSpeed);
            transform.position = new Vector3(Mathf.Clamp(cameraPos.x, m_screenXSizeLimit.x, m_screenXSizeLimit.y),
                Mathf.Clamp(cameraPos.y, m_screenYSizeLimit.x, m_screenYSizeLimit.y), cameraPos.z);
