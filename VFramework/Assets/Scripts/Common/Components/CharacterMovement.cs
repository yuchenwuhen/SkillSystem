...
namespace VFramework.Character
{
    ...
    public sealed class CharacterMovement : MonoBehaviour
    {
        ...
        private void PreventGroundPenetration()
        {
            ...
            remainingLateralVelocity = MathTool.GetTangent(remainingLateralVelocity, hitInfo.normal, up) * remainingLateralVelocity.magnitude;
        ...
        private void ApplyMovement(Vector3 desiredVelocity, float maxDesiredSpeed, bool onlyLateral)
        {
            ...
            if (isGrounded)
            {
                if (!slideOnSteepSlope || groundAngle < slopeLimit)
                {
                    ...
                    desiredVelocity = MathTool.GetTangent(desiredVelocity, _normal, up) * Mathf.Min(desiredVelocity.magnitude, maxDesiredSpeed);
            ...
            float deceleration, float friction, float brakingFriction)
        {
            ...
            if (!slideOnSteepSlope || groundAngle < slopeLimit)
            {
                ...
                var desiredDirection = MathTool.GetTangent(desiredVelocity, _normal, up);
                ...
                if (desiredAcceleration.IsZero() || v.IsExceeding(speedLimit))
                {
                    ...
                    v = MathTool.GetTangent(v, _normal, up) * v.magnitude;
        ...
        private void SnapToPlatform(ref Vector3 probingPosition, ref Quaternion probingRotation)
        {
            ...
            if (hitInfo.isOnLedgeSolidSide)
                groundedPosition = MathTool.ProjectPointOnPlane(groundedPosition, hitInfo.groundPoint, up);
