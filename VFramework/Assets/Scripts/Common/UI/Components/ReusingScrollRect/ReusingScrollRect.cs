...
namespace VFramework.UI
{
    public class ReusingScrollRect : ScrollRectInput
    {
        ...
        public void SetItem(string itemName)
        {
            ...
            m_itemPrefab = GameObjectPool.Instance.CreateObject(m_ItemName);
        ...
        public override void Dispose()
        {
            ...
            {
                GameObjectPool.Instance.CollectObject(m_items[i].gameObject);
            }
            m_items.Clear();

            for (int i = 0; i < m_itemCaches.Count; i++)
            {
                GameObjectPool.Instance.CollectObject(m_itemCaches[i].gameObject);
            }
            m_itemCaches.Clear();
        }
        ...
        ReusingScrollItemBase GetItem()
        {
            ...
            result = GameObjectPool.Instance.CreateObject(m_itemPrefab).GetComponent<ReusingScrollItemBase>();
